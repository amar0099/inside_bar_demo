...
def detect_inside_bar(df: pd.DataFrame) -> list[InsideBarSetup]:
    ...
    for i in range(1, len(df)):
        ...
        if is_first_candle(m["datetime"]) or is_first_candle(b["datetime"]):
            continue
        # Skip if baby candle is the last candle of the day —
        # breakout would trigger on next day's first candle which we don't want
...
def generate_signals(df: pd.DataFrame,
    ...
    for setup in setups:
        ...
        for k in range(setup.baby_idx + 1,
            ...
            c = df.iloc[k]
            # Do not trade if breakout candle is on a different day than the setup
