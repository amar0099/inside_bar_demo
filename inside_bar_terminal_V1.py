...
def compute_daily_ranges(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate intraday candles into per-day OHLC + range_pct."""
...
def regime_allowed(df: pd.DataFrame, target_date) -> tuple[bool, float | None]:
    """
    ...
    If target_date falls inside the dataset (i.e. you're checking a past day for
    backtest purposes), the returned avg correctly excludes target_date itself.

    Returns (True, None) if fewer than REGIME_LOOKBACK_DAYS prior days are available.
    """
    if not REGIME_FILTER_ENABLED:
        return True, None
    daily = compute_daily_ranges(df)
    if daily.empty:
        return True, None
...
def regime_diagnostics(df: pd.DataFrame, target_date) -> dict:
    """
    ...
    daily = compute_daily_ranges(df)
    if daily.empty:
        return {"status": "no_data", "available_days": 0, "prior_days": [], "avg": None}
...
def generate_signals(df: pd.DataFrame,
    ...
    for setup in setups:
        ...
        regime_ok, _ = regime_allowed(df, setup_date)
        if not regime_ok:
            continue
...
class DemoEngine:
    ...
    def can_accept_signal(self, signal: TradeSignal,
                          spot_df: Optional[pd.DataFrame] = None) -> tuple[bool, str]:
        """v2 gates: returns (allowed, reason_if_not).

        If spot_df is provided, also re-checks the regime gate using prior days
        before the signal's date — defends against stale signals if the data
        has been refreshed since signals were generated.
        """
        ...
        # Regime re-check (only if spot_df given)
        if spot_df is not None:
            ok, _avg = regime_allowed(spot_df, sig_date)
            if not ok:
    ...
    for i in range(cursor, end):
        ...
        for sig in st.session_state.signals:
            if sig.signal_candle_idx == i:
                ...
                ok, reason = st.session_state.engine_spot.can_accept_signal(sig, spot_df=df)
                if not ok:
