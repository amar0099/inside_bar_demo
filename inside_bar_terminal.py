...
.stApp { background: #0a0c0f; color: #c8d0dc; }
[data-testid="stSidebar"] { background: #0e1117 !important; border-right: 1px solid #1e2530; }

h1, h2, h3 { color: #e8edf3 !important; font-family: 'IBM Plex Mono', monospace !important; }
h1 { font-size: 1.3rem !important; letter-spacing: 0.08em; }
h2 { font-size: 1.0rem !important; letter-spacing: 0.06em; color: #7eb8f7 !important; }

[data-testid="stMetric"] { background:#111520; border:1px solid #1e2a38; border-radius:4px; padding:12px 16px; }
...
[data-testid="stMetricValue"] { color:#e8edf3 !important; font-family:'IBM Plex Mono',monospace !important; font-size:1.4rem !important; }

.stButton > button { background:#1a2540 !important; color:#7eb8f7 !important; border:1px solid #2a3f60 !important; border-radius:3px !important; font-family:'IBM Plex Mono',monospace !important; font-size:0.75rem !important; letter-spacing:0.08em !important; text-transform:uppercase !important; padding:6px 16px !important; transition:all 0.15s !important; }
.stButton > button:hover { background:#243558 !important; border-color:#4a7ab8 !important; color:#afd4ff !important; }

.stSelectbox > div > div, .stNumberInput > div > div { background:#111520 !important; border-color:#1e2a38 !important; color:#c8d0dc !important; font-family:'IBM Plex Mono',monospace !important; font-size:0.8rem !important; }
.stRadio label { color:#8a9ab8 !important; font-size:0.8rem !important; }
hr { border-color:#1e2530 !important; }
.dataframe { background:#0e1117 !important; font-family:'IBM Plex Mono',monospace !important; font-size:0.72rem !important; border:1px solid #1e2530 !important; }
.dataframe th { background:#111520 !important; color:#5a7a9a !important; }
.dataframe td { color:#c8d0dc !important; border-color:#1a2030 !important; }

.section-label { font-family:'IBM Plex Mono',monospace; font-size:0.65rem; color:#3a5a7a; letter-spacing:0.15em; text-transform:uppercase; border-bottom:1px solid #1e2530; padding-bottom:4px; margin-bottom:10px; margin-top:14px; }
.trade-card { background:#0e1520; border:1px solid #1e2a38; border-left:3px solid #2a6090; padding:10px 14px; margin:6px 0; border-radius:3px; font-family:'IBM Plex Mono',monospace; font-size:0.75rem; color:#c8d0dc; }
...
def get_fyers_client():
    """
    Auto-login using all credentials from Streamlit secrets.
    Token is cached for 1 hour (Fyers tokens are valid all day).
    """
    try:
        from fyers_apiv3 import fyersModel
        cfg = st.secrets["fyers"]
...
def build_ohlc_chart(df: pd.DataFrame, setups: list[InsideBarSetup],
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing=dict(line=dict(color="#3ddc84", width=1), fillcolor="#0d3320"),
        decreasing=dict(line=dict(color="#ff6b6b", width=1), fillcolor="#330d0d"),
    ), row=1, col=1)

    vol_colors = ["#1a4d28" if ddf["close"].iloc[i] >= ddf["open"].iloc[i]
                  else "#4d1a1a" for i in range(len(ddf))]
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#0a0c0f", paper_bgcolor="#0a0c0f",
    ...
    for ax in [fig.layout.xaxis, fig.layout.xaxis2,
        ...
        if hasattr(ax, "tickfont"):
            ax.tickfont = dict(family="IBM Plex Mono", size=9, color="#4a6a8a")
    return fig
...
def build_pnl_chart(closed: list[LiveTrade]) -> go.Figure | None:
    ...
    clrs = ["#1a4d28" if p >= 0 else "#4d1a1a" for p in pnls]
    fig  = go.Figure()
    fig.add_trace(go.Bar(x=list(range(len(pnls))), y=pnls, marker_color=clrs,
                         name="Trade P&L", text=[f"{p:+.1f}" for p in pnls],
                         textposition="outside",
                         textfont=dict(size=9, family="IBM Plex Mono", color="#c8d0dc")))
    fig.add_trace(go.Scatter(x=list(range(len(cum))), y=cum, mode="lines",
                             name="Cumulative", line=dict(color="#7eb8f7", width=2)))
    fig.update_layout(plot_bgcolor="#0a0c0f", paper_bgcolor="#0a0c0f",
                      font=dict(family="IBM Plex Mono", color="#5a7a9a", size=10),
                      height=230, margin=dict(l=10, r=10, t=10, b=30),
                      legend=dict(font=dict(size=9), bgcolor="#0e1117", bordercolor="#1e2530"))
    fig.update_xaxes(gridcolor="#131820")
    fig.update_yaxes(gridcolor="#131820", ticksuffix=" pts")
    return fig
...
with st.sidebar:
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:0.65rem;color:#3a5a7a;'
    ...
    if is_demo:
        ...
        st.markdown(
            ...
            '<span style="color:#3a5a7a">─────────────────────</span><br>'
            '<b style="color:#c8d0dc">[fyers]</b><br>'
    ...
    if st.session_state.atm_strike:
        ...
        st.markdown(
            f'<div style="font-family:IBM Plex Mono;font-size:0.7rem;color:#c8d0dc;line-height:2.1;">'
            f'NIFTY 50 SPOT<br>'
...
st.markdown(
    f'<div style="font-family:IBM Plex Mono;font-size:1.1rem;color:#e8edf3;'
...
with tab1:
    if df_s is None:
        st.markdown(
            '<div style="text-align:center;padding:70px 0;color:#3a5a7a;font-family:IBM Plex Mono;">'
            '<div style="font-size:2rem;margin-bottom:14px;">◫</div>'
            '<div style="font-size:0.9rem;letter-spacing:0.1em;">NO DATA LOADED</div>'
            '<div style="font-size:0.75rem;margin-top:8px;color:#2a4060;">'
        ...
        with cc5:
            ...
            st.markdown(
                f'<div style="font-family:IBM Plex Mono;font-size:0.7rem;color:#4a6a8a;padding-top:8px;">'
        ...
        if active:
            ...
            for t in active:
                ...
                st.markdown(
                    ...
                    f'<span style="color:#7eb8f7">{t.instrument}</span> &nbsp;'
...
with tab3:
    ...
    if bt is None:
        ...
        fig_bt.add_trace(go.Scatter(x=list(range(len(cum))), y=cum, mode="lines",
                                    name="Cumulative", line=dict(color="#7eb8f7", width=2)))
        fig_bt.update_layout(plot_bgcolor="#0a0c0f", paper_bgcolor="#0a0c0f",
                             font=dict(family="IBM Plex Mono", color="#5a7a9a", size=10),
                             height=270, margin=dict(l=10,r=10,t=10,b=30),
                             legend=dict(font=dict(size=9), bgcolor="#0e1117", bordercolor="#1e2530"))
        fig_bt.update_xaxes(gridcolor="#131820")
        fig_bt.update_yaxes(gridcolor="#131820", ticksuffix=" pts")
    ...
    if not msgs:
        ...
        for m in msgs:
            clr = ("#ff6b6b" if "ERROR" in m else
                   "#7eb8f7" if "⚡" in m else
                   "#3ddc84" if "✅" in m else "#4a6a8a")
            lines += f'<div style="color:{clr}">{m}</div>'
        st.markdown(
            f'<div style="font-family:IBM Plex Mono;font-size:0.72rem;line-height:2;'
            f'background:#0e1117;padding:12px 16px;border-radius:4px;'
            f'border:1px solid #1e2530;max-height:500px;overflow-y:auto;">{lines}</div>',
            unsafe_allow_html=True,
        )
