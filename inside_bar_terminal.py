...
def regime_allowed(df: pd.DataFrame, target_date) -> tuple[bool, float | None]:
    ...
    Uses ONLY the previous REGIME_LOOKBACK_DAYS completed trading days
    (no look-ahead). Returns (True, None) if not enough history available.
    """
    ...
    if len(prior) < REGIME_LOOKBACK_DAYS:
        # Not enough history — allow trading (avoid blocking the day on first run)
        return True, None
...
class DemoEngine:
    ...
    def can_accept_signal(self, signal: TradeSignal) -> tuple[bool, str]:
    ...
    for i in range(cursor, end):
        ...
        for sig in st.session_state.signals:
            if sig.signal_candle_idx == i:
                # v2: ask the engine if it can accept this signal (one-trade-at-a-time + daily SL)
                ok, reason = st.session_state.engine_spot.can_accept_signal(sig)
                if not ok:
    ...
    if st.session_state.spot_df is not None and not st.session_state.spot_df.empty:
        try:
            ...
            st.markdown(
                ...
                f'font-size:0.68rem;letter-spacing:0.08em;text-align:center;">{status}</div>',
                unsafe_allow_html=True,
            )
        except Exception:
            pass
